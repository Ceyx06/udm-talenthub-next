...
export async function GET(request: NextRequest) {
  try {
    ...
    const timeToFillData = hiredApps.map(app => {
      ...
      // Fixed: Changed 'hiredDate' to 'hiredAt'
    ...
    const avgTimeToFill = timeToFillData.length > 0
      ? Math.round(timeToFillData.reduce((sum, d) => sum + d.days, 0) / timeToFillData.length)
      : 0;

    // Group by month for trend
    // Fixed: Removed space in function name 'getL ast6Months' -> 'getLast6Months'
    ...
    const trendData = last6Months.map(monthKey => {
      ...
      const avgDays = monthData.length > 0
        ? Math.round(monthData.reduce((sum, d) => sum + d.days, 0) / monthData.length)
    ...
    const deptPerformance = Object.entries(deptStats).map(([dept, days]) => ({
    ...
    const criticalSteps = processSteps.filter(s => s.status === 'critical').length;
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        process: {
          steps: processSteps,
          criticalSteps,
          totalProcessTime: processSteps.reduce((sum, s) => sum + s.avgDays, 0),
        },
        predictions,
        stats: {
          totalApplications: applications.length,
          totalHired: hiredApps.length,
          inProgress: applications.filter(a => !['HIRED', 'REJECTED'].includes(a.stage || '')).length,
        },
      },
    });
  } catch (error: any) {
...
async function calculateProcessSteps(applications: any[]) {
  const steps = [
    ...
    { step: 'Screen → Interview', avgDays: 0, status: 'good' as const, count: 0 },
    { step: 'Interview → Evaluation', avgDays: 0, status: 'good' as const, count: 0 },
    { step: 'Evaluation → Offer', avgDays: 0, status: 'good' as const, count: 0 },
  ...
  applications.forEach(app => {
    ...
    // Post → Screen (time from application to PENDING/UNDER_REVIEW)
    if (app.screenedDate) {
      const screenedDate = new Date(app.screenedDate);
      const days = Math.ceil((screenedDate.getTime() - created.getTime()) / (1000 * 60 * 60 * 24));
    ...
    if (app.interviews && app.interviews.length > 0) {
      ...
      const screenedDate = app.screenedDate ? new Date(app.screenedDate) : created;
      const days = Math.ceil((firstInterview.getTime() - screenedDate.getTime()) / (1000 * 60 * 60 * 24));
    ...
    if (app.evaluation && app.interviews && app.interviews.length > 0) {
      const lastInterview = new Date(app.interviews[app.interviews.length - 1].scheduledDate);
      const evaluatedDate = new Date(app.evaluation.evaluatedAt);
      const days = Math.ceil((evaluatedDate.getTime() - lastInterview.getTime()) / (1000 * 60 * 60 * 24));
    ...
    if (app.evaluation?.contractStatus === 'approved') {
      ...
      const approvedDate = app.evaluation.contractActionDate
        ...
        : new Date();
      const days = Math.ceil((approvedDate.getTime() - evaluatedDate.getTime()) / (1000 * 60 * 60 * 24));
    ...
    if (app.stage === 'HIRED' && app.hiredAt) {
      ...
      const contractDate = app.evaluation?.contractActionDate
        ? new Date(app.evaluation.contractActionDate)
        : created;
      const days = Math.ceil((hiredDate.getTime() - contractDate.getTime()) / (1000 * 60 * 60 * 24));
...
async function calculatePredictions(applications: any[]) {
  ...
  // Fixed: Changed 'hiredDate' to 'hiredAt'
  const hiredApps = applications.filter(app => app.stage === 'HIRED' && app.hiredAt);
  ...
  // Get top 3 most common positions
  const predictions = Object.entries(deptStats)
    .map(([position, days]) => {
      const avg = Math.round(days.reduce((sum, d) => sum + d, 0) / days.length);
  ...
  return predictions.length > 0 ? predictions : [
    { position: 'No historical data', predictedDays: 15, confidence: 50, sampleSize: 0 },
  ];
}